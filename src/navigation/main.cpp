...
int main(){
  ...
  MatrixXf R = MatrixXf::Random(m*knumimus,m*knumimus);
  ...
  for(std::size_t i=0; i < data1.size(); ++i)
  {
    ...
    for(int j = 0; j < m; j++)
    {
      ...
      {
        z(j*knumimus + imu) = IMUs[imu][i][j+1];
      }
    }
    ...
    if (i == 0){
      ...
      for(int imu = 0; imu < knumimus; imu++)
      {
        dt = dt + (float)0.001*(IMUs[imu][i][0] - IMUs[imu][i-1][0]);
      }

      dt = dt / (float)knumimus;
    ...
    for(int j = 0; j < n; j++)
    {
      fout << KF.get_state()(j) << std::setw(15);
