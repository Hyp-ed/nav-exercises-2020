...
class ManagerInterface : public Thread {
 public:
   ...
   *
   */
  explicit ManagerInterface(uint8_t id, utils::Logger& log) : Thread(id, log), old_timestamp_(0) {}
 protected:
...
class ImuManagerInterface : public ManagerInterface {
 public:
  explicit ImuManagerInterface(uint8_t id, utils::Logger& log) : ManagerInterface(id, log) {}
};
